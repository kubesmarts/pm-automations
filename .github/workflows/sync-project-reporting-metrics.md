...
1. **The JIRA ticket has the label `gh-sync`** — this label must be present on the JIRA issue; tickets without it are skipped.
...
- The referenced JIRA ticket has the label `gh-sync`
...
6. **Verify JIRA sync (if configured)** on the linked ticket:
   - The Actions log shows `Syncing to JIRA ticket: <id>` (confirming the `gh-sync` label is present)
...
- **JIRA sync skipped with "does not have the 'gh-sync' label"** → add the `gh-sync` label to the JIRA ticket to opt it in to syncing
